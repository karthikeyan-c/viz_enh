...
const nameRenderer = function (data) {
  return (<span className={data.rowData.className} title={data.rowData.name}>{data.rowData.tName}-v1</span>);
};

const twoxxRenderer = function (data) {
  return (<span className={data.rowData.className} title={data.rowData.normals}>{data.rowData.normals}</span>);
};

const fourxxRenderer = function (data) {
  return (<span className={data.rowData.className} title={data.rowData.warnings}>{data.rowData.warnings}</span>);
};

const fivexxRenderer = function (data) {
  return (<span className={data.rowData.className} title={data.rowData.errors}>{data.rowData.errors}</span>);
};
...
class SubNodeList extends React.Component {
  ...
  render () {
    ...
    const nodeRows = _.map(this.state.nodes, (node) => {
      ...
      const warnings = (node.metrics && node.metrics.warning) || 0;
      const normals = (node.metrics && node.metrics.normal) || 0;
      const t = (node.name).split("-");
      const tName = t[2];
      ...
      nodeRows.length > 0
        ? <div className="node-list">
          <Table
            ...
            sort={this.sort}
          >
            <Column label="Name" dataKey="name" cellRenderer={nameRenderer} width={100} />
            <Column label="2XX" dataKey="errorRate" cellRenderer={twoxxRenderer} width={82}/>
            <Column label="4XX" dataKey="errorRate" cellRenderer={fourxxRenderer} width={82}/>
            <Column label="5XX" dataKey="errorRate" cellRenderer={fivexxRenderer} width={82}/>
            <Column label="Dist" dataKey="totalPercent" cellRenderer={totalRenderer} width={82}/>
